<div class="space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-navy-900 tracking-tight">Add News Article</h1>
            <p class="text-gray-500 mt-1 font-medium">Create a new article for the media center.</p>
        </div>
    </div>

    <form action="<?php echo url('media/store'); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <?php echo UI::card("Article Content", "
                    <div class='grid grid-cols-1 gap-6'>
                        " . UI::bilingualInput('title', 'Headline', [], true) . "
                    ...
                    <div class='grid grid-cols-1 gap-6'>
                        ...
                        <div class='flex flex-col gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100'>
                            <label class='flex items-center gap-3 cursor-pointer'>
                                <input type='checkbox' name='is_featured' class='w-5 h-5 rounded text-teal-600 focus:ring-teal-500'>
                                <span class='text-sm font-bold text-gray-700'>Feature this article</span>
                            </label>
                            <label class='flex items-center gap-3 cursor-pointer'>
                                <input type='checkbox' name='is_active' checked class='w-5 h-5 rounded text-teal-600 focus:ring-teal-500'>
