<div class="space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-navy-900 tracking-tight">Edit Announcement</h1>
            <p class="text-gray-500 mt-1 font-medium">Update announcement details and settings.</p>
        </div>
    </div>

    <form action="<?php echo url('announcements/update/' . $announcement['id']); ?>" method="POST"
        enctype="multipart/form-data" class="space-y-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <?php echo UI::card("Announcement Content", "
                    <div class='grid grid-cols-1 gap-6'>
                        " . UI::bilingualInput('title', 'Title', ['en' => $announcement['title_en'], 'so' => $announcement['title_so']], true) . "
                        " . UI::bilingualTextarea('description', 'Description', ['en' => $announcement['description_en'], 'so' => $announcement['description_so']], true, 6, true) . "
                    </div>
                "); ?>

                <?php echo UI::card("Call to Action (Optional)", "
                    <div class='grid grid-cols-1 md:grid-cols-2 gap-6'>
                        " . UI::bilingualInput('action_label', 'Button Label', ['en' => $announcement['action_label_en'], 'so' => $announcement['action_label_so']]) . "
                        " . UI::input('action_link', 'Button Link', 'text', $announcement['action_link'], 'e.g. https://... or internal path') . "
                    </div>
                "); ?>
            </div>
            ...
            <div class="space-y-6">
                <?php echo UI::card("Settings", "
                    <div class='grid grid-cols-1 gap-6'>
                        " . UI::input('published_date', 'Publication Date', 'date', $announcement['published_date'], '', true) . "
                        <div class='space-y-2'>
                            ...
                            <select name='status' class='w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 outline-none transition-all'>
                                <option value='open' " . ($announcement['status'] == 'open' ? 'selected' : '') . ">Open</option>
                                <option value='closed' " . ($announcement['status'] == 'closed' ? 'selected' : '') . ">Closed</option>
                        ...
                        <div class='space-y-2'>
                            ...
                            <select name='border_color' class='w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 outline-none transition-all'>
                                <option value='gold-500' " . ($announcement['border_color'] == 'gold-500' ? 'selected' : '') . ">Gold (Default)</option>
                                <option value='teal-500' " . ($announcement['border_color'] == 'teal-500' ? 'selected' : '') . ">Teal</option>
                                <option value='navy-900' " . ($announcement['border_color'] == 'navy-900' ? 'selected' : '') . ">Navy</option>
                                <option value='red-500' " . ($announcement['border_color'] == 'red-500' ? 'selected' : '') . ">Red</option>
                            </select>
                        </div>
                        " . UI::input('sort_order', 'Sort Order', 'number', $announcement['sort_order']) . "
                    </div>
                "); ?>
                ...
                <?php echo UI::card("Attachment", "
                    <div class='space-y-2'>
                        ...
                        " . ($announcement['attachment'] ? "<div class='mb-2 text-xs text-teal-600 font-medium'>Current: {$announcement['attachment']}</div>" : "") . "
