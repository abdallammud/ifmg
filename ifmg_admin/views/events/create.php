<div class="space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-navy-900 tracking-tight">Add Event</h1>
            <p class="text-gray-500 mt-1 font-medium">Create a new workshop, conference or seminar entry.</p>
        </div>
    </div>

    <form action="<?php echo url('events/store'); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <?php echo UI::card("Event Information", "
                    <div class='grid grid-cols-1 gap-6'>
                        " . UI::bilingualInput('title', 'Event Title', [], true) . "
                        " . UI::bilingualTextarea('description', 'Description', [], false, 4, true) . "
                        " . UI::bilingualInput('location', 'Location', [], true) . "
                    </div>
                "); ?>
            </div>
            ...
            <div class="space-y-6">
                <?php echo UI::card("Date & Type", "
                    <div class='grid grid-cols-1 gap-6'>
                        " . UI::input('event_date', 'Event Date', 'date', date('Y-m-d'), '', true) . "
                        <div class='grid grid-cols-2 gap-4'>
                            " . UI::input('start_time', 'Start Time', 'time') . "
                            " . UI::input('end_time', 'End Time', 'time') . "
                        </div>
                        <div class='space-y-2'>
