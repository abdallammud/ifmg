<div class="max-w-4xl mx-auto py-10">
    <div class="mb-8 flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-navy-900">Edit Core Value</h1>
    </div>

    <form action="<?php echo url('core-values/update/' . $value['id']); ?>" method="POST" class="space-y-6">
        <?php echo UI::card("Core Value Details", "
            <div class='grid grid-cols-1 gap-6'>
                " . UI::bilingualInput('title', 'Value Title', ['en' => $value['title_en'], 'so' => $value['title_so']], true) . "
                " . UI::bilingualTextarea('description', 'Description', ['en' => $value['description_en'], 'so' => $value['description_so']], false, 4) . "
                " . UI::input('sort_order', 'Sort Order', 'number', $value['sort_order']) . "
            </div>
        "); ?>
