    ...
    <div class="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div class="grid lg:grid-cols-4 gap-12">
            ...
            <div class="lg:col-span-3">
                <span class="inline-block text-emerald-600 font-semibold text-sm uppercase tracking-widest mb-3"
                    ...
                    <?php if (isset($features_res) && $features_res->num_rows > 0): ?>
                        <?php while ($f = $features_res->fetch_assoc()): ?>
                            <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                                <div
                                    class="w-14 h-14 rounded-xl <?php echo e($f['icon_bg_color'] ?? 'bg-emerald-500/10'); ?> flex items-center justify-center mb-6">
                                    <div class="text-<?php echo e($f['icon_color'] ?? 'emerald-600'); ?>">
                ...
                <div class="bg-navy-50 rounded-2xl p-8 border border-navy-100">
                    <h3 class="text-xl font-semibold text-navy-900 mb-4" data-translate="research_priority">Research
                        Priorities</h3>
                        ...
                        <?php if (isset($items_res) && $items_res->num_rows > 0): ?>
                            <?php while ($item = $items_res->fetch_assoc()): ?>
                                <li class="flex items-center gap-2">
                                    <span
                                        class="w-2 h-2 rounded-full bg-<?php echo e($item['bullet_color'] ?? 'emerald-500'); ?>"></span>
